...
@section('content')
    <section class="py-8 sm:py-12">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {{-- Tab Navigation --}}
            <div class="mb-6">
                <div class="flex gap-2 p-1.5 rounded-2xl"
                    style="background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.1); backdrop-filter: blur(12px);">
                   ...
                   {{ $tab === 'active' ? 'text-white shadow-lg' : 'text-white/60 hover:text-white hover:bg-white/10' }}"
                        style="{{ $tab === 'active'
                    ...
                    <a href="{{ route('my.orders', ['tab' => 'history']) }}"
                        class="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl font-semibold text-sm transition-all duration-300
                   {{ $tab === 'history' ? 'text-white shadow-lg' : 'text-white/60 hover:text-white hover:bg-white/10' }}"
                        style="{{ $tab === 'history'
                            ? 'background: linear-gradient(135deg, #0891b2 0%, #14b8a6 100%); box-shadow: 0 4px 15px rgba(6,182,212,0.4);'
                            : '' }}">
                    ...
                    @foreach ($orders as $order)
                        <div class="store-glass-card rounded-2xl overflow-hidden">
                            ...
                            <div class="p-5 sm:p-6 border-b border-white/10">
                                ...
                                {{ match ($order->status) {
                                    'pending' => 'bg-sky-500/20 text-sky-300 border border-sky-500/30',
                                    'waiting_payment' => 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30',
                                ...
                                <div class="space-y-3">
                                    @foreach ($order->items as $item)
                                        <div class="flex items-center gap-3 sm:gap-4">
                                            <div
                                                class="w-12 h-12 sm:w-14 sm:h-14 rounded-xl overflow-hidden flex-shrink-0 bg-white/10 border border-white/10">
                                                @if ($item->produk->foto)
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <p class="font-semibold text-white text-sm truncate">
                                                    {{ $item->nama_produk ?? ($item->produk?->nama ?? 'Produk Dihapus') }}
                                                </p>
                                                <div class="flex items-center gap-2 mt-0.5">
                                                    <span
                                                        class="{{ $item->produk->kategori === 'Ikan Nila' ? 'badge-nila' : 'badge-mas' }} text-[10px] px-2 py-0.5">
                                                        {{ $item->produk->kategori }}
                                                    </span>
                                                    <span class="text-xs text-white/50">{{ number_format($item->qty, 1) }}
                                                        Kg</span>
                                                </div>
                                            </div>
                                    ...
                                    <div class="mt-4 pt-4 border-t border-white/10">
                                        ...
                                        <div class="grid grid-cols-1 gap-2">
                                            @foreach ($order->items as $item)
                                                ...
                                                @endphp
                                                <div
                                                    class="flex items-center justify-between gap-3 p-3 rounded-xl {{ $hasReview ? 'bg-mint-500/10 border border-mint-500/20' : 'bg-white/5 border border-white/10' }}">
                                                    <div class="flex items-center gap-2 flex-1 min-w-0">
                                                        <div
                                                            class="w-8 h-8 rounded-lg overflow-hidden flex-shrink-0 bg-white/10">
                                                            @if ($item->produk->foto)
                                                                <img loading="lazy"
                                                    ...
                                                    @else
                                                        <a href="{{ route('review.create', ['order' => $order->id, 'produk' => $item->produk->id]) }}"
                                                            ...
                                                            style="background: linear-gradient(135deg, #0ea5e9 0%, #14b8a6 100%); box-shadow: 0 2px 8px rgba(14,165,233,0.35);">
                            ...
                            @if ($order->status === 'pending' && $order->payment_deadline)
                                <div class="px-5 sm:px-6 pb-0 bg-white/5">
                                    <div
                                        class="{{ $order->isPaymentExpired() ? 'bg-red-500/15 border-red-500/30' : 'bg-sky-500/15 border-sky-500/30' }} border rounded-xl p-3 flex items-center gap-2">
                                        <i
                                            class="fas {{ $order->isPaymentExpired() ? 'fa-exclamation-circle text-red-400' : 'fa-clock text-cyan-300' }}"></i>
                                        <span
                                            class="text-xs font-medium {{ $order->isPaymentExpired() ? 'text-red-300' : 'text-cyan-200' }}">
                            ...
                            @if ($order->refund_status !== 'none')
                                <div class="px-5 sm:px-6 pb-0 bg-white/5">
                                    @if ($order->refund_status === 'requested')
                                        <div class="bg-sky-500/15 border border-sky-500/30 rounded-xl p-3">
                                            <div class="flex items-start gap-2">
                                                <i class="fas fa-clock text-cyan-300 mt-0.5"></i>
                                                <div class="flex-1">
                                                    <p class="text-xs font-bold text-cyan-200">Permintaan Refund Sedang
                                                        Diproses</p>
                                                    <p class="text-xs text-cyan-300/80 mt-1"><strong>Alasan:</strong>
                                                        {{ $order->refund_reason }}</p>
                                                    <p class="text-[10px] text-white/40 mt-1.5">Diajukan:
                                                        {{ $order->refund_requested_at?->format('d M Y, H:i') }}</p>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div class="flex items-start gap-2">
                                                ...
                                                <div class="flex-1">
                                                    ...
                                                    <p class="text-xs text-green-400/80 mt-1">
                                                        {{ $order->refund_admin_note }}</p>
                                                    <p class="text-[10px] text-white/40 mt-1.5">Diproses:
                                                        {{ $order->refund_processed_at?->format('d M Y, H:i') }}</p>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="bg-red-500/15 border border-red-500/30 rounded-xl p-3">
                                            <div class="flex items-start gap-2">
                                                ...
                                                <div class="flex-1">
                                                    ...
                                                    <p class="text-xs text-red-400 mt-1"><strong>Alasan penolakan:</strong>
                                                        {{ $order->refund_admin_note }}</p>
                                                    <p class="text-[10px] text-white/40 mt-1.5">Diproses:
                                                        {{ $order->refund_processed_at?->format('d M Y, H:i') }}</p>
                                                </div>
                                ...
                                <div class="px-5 sm:px-6 pb-0 bg-white/5">
                                    <div class="bg-cyan-500/10 border border-cyan-500/20 rounded-xl p-4">
                                        <div class="flex items-start gap-3">
                                            <div
                                                class="w-8 h-8 rounded-lg bg-ocean-500 flex items-center justify-center flex-shrink-0">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                ...
                                                <p class="text-xs text-white/70 leading-relaxed mb-2">
                                                    {{ $order->delivery_note }}</p>
                                                @if ($order->delivery_time)
                                                    <p
                                                        class="text-[10px] text-cyan-400 font-semibold flex items-center gap-1">
                                                        <i class="fas fa-clock"></i>
                                                        {{ \Carbon\Carbon::parse($order->delivery_time)->format('d M Y, H:i') }}
                                                        WIB
                                                    </p>
                                                @endif
                                                    ...
                                                    <div class="mt-3 pt-3 border-t border-white/10">
                                                        ...
                                                        <div class="grid grid-cols-2 gap-2">
                                                            <div class="flex flex-col">
                                                                ...
                                                                <span
                                                                    class="text-xs font-semibold text-white">{{ $order->courier_name }}</span>
                                                            </div>
                                                            @if ($order->courier_phone)
                                                                <div class="flex flex-col">
                                                                    <span class="text-[10px] text-white/50">HP</span>
                                                                    <a href="tel:{{ $order->courier_phone }}"
                                                                        class="text-xs font-semibold text-cyan-300 underline hover:text-cyan-200">
                                                                        {{ $order->courier_phone }}
                                                                    </a>
                                                                </div>
                                                            @endif
                                                            @if ($order->tracking_number)
                                                                <div class="flex flex-col col-span-2">
                                                                    <span
                                                                        class="text-[10px] text-white/50">Resi/Plat</span>
                                                                    <span
                                                                        class="text-xs font-semibold text-white">{{ $order->tracking_number }}</span>
                                                                </div>
                                                            @endif
                                                        </div>
                                                    </div>
                                                ...
                                                <div class="mt-3 pt-3 border-t border-white/10">
                                                    <a href="{{ route('order.track', $order) }}"
                                                        class="text-xs font-semibold text-cyan-300 hover:text-cyan-200 flex items-center gap-1">
                                ...
                                <div class="px-5 sm:px-6 pb-3 sm:pb-4 bg-white/5">
                                    <div class="pt-4 border-t border-white/10">
                                        ...
                                        <div class="flex items-center justify-between relative">
                                            {{-- Progress Line --}}
                                            <div class="absolute top-4 left-4 right-4 h-1 rounded-full bg-white/10 -z-10">
                                            </div>
                                            <div class="absolute top-4 left-4 h-1 rounded-full -z-10 transition-all duration-500"
                                                style="width: {{ $currentStep * 20 }}%; background: linear-gradient(90deg, #0891b2 0%, #14b8a6 100%);">
                                            </div>
                                    ...
                                    {{ $isStepDone ? 'text-white' : ($isStepCurrent ? 'text-white ring-2 ring-cyan-400/50' : 'bg-white/10 text-white/40') }}"
                                                    ...
                                                    <span
                                                        class="text-[8px] sm:text-[10px] mt-1.5 {{ $isStepDone ? 'text-cyan-300 font-semibold' : ($isStepCurrent ? 'text-sky-300 font-semibold' : 'text-white/30') }} text-center leading-tight">
                                                        {{ $label }}
                                                    </span>
                                                </div>
                                            @endforeach
                            ...
                            @else
                                <div class="px-5 sm:px-6 pb-3 sm:pb-4 bg-white/5">
                                    <div
                                        class="pt-4 border-t border-white/10 flex items-center justify-center gap-2 text-red-400">
                                ...
                                <div class="flex flex-col sm:flex-row gap-2 sm:gap-3">
                                    ...
                                    @if ($order->status === 'pending' && !$order->isPaymentExpired())
                                        <a href="{{ route('order.success', $order) }}"
                                            ...
                                            style="background: linear-gradient(135deg, #0ea5e9 0%, #14b8a6 100%); box-shadow: 0 4px 15px rgba(14,165,233,0.35);">
                                    ...
                                    @if ($order->status === 'pending' && $order->isPaymentExpired())
                                        <div
                                            class="flex-1 flex items-center justify-center gap-2 px-5 py-3.5 rounded-xl font-bold text-sm text-red-300 bg-red-500/15 border border-red-500/30 cursor-not-allowed">
                                    ...
                                    @if (in_array($order->status, ['pending', 'waiting_payment', 'paid', 'confirmed']) && $order->refund_status === 'none')
                                        <button type="button"
                                            onclick="openRefundModal({{ $order->id }}, '{{ $order->order_number }}')"
                                            class="flex-shrink-0 flex items-center justify-center gap-2 px-5 py-3.5 rounded-xl font-bold text-sm text-cyan-300 transition-all duration-300 active:scale-[0.97] touch-manipulation"
            ...
            @else
                <div class="text-center py-20">
                    @if ($tab === 'active')
                        ...
                        <p class="text-white/50 max-w-md mx-auto mb-6">Semua pesanan Anda sudah selesai diproses. Yuk
                            belanja ikan segar lagi!</p>
                        <div class="flex flex-col sm:flex-row gap-3 justify-center">
                            ...
                            @if ($historyCount > 0)
                                <a href="{{ route('my.orders', ['tab' => 'history']) }}"
                                    class="inline-flex items-center justify-center gap-2 px-6 py-3 font-semibold rounded-xl border border-white/20 text-white bg-white/10 backdrop-blur hover:bg-white/15 transition-all">
                    ...
                    @else
                        ...
                        <p class="text-white/50 max-w-md mx-auto mb-6">Pesanan yang sudah selesai atau dibatalkan akan
                            muncul di sini.</p>
                        <a href="{{ route('my.orders', ['tab' => 'active']) }}"
                            class="inline-flex items-center justify-center gap-2 px-6 py-3 font-semibold rounded-xl border border-white/20 text-white bg-white/10 backdrop-blur hover:bg-white/15 transition-all">
    ...
    {{-- Modal Refund --}}
    <div id="refundModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4"
        style="background: rgba(0,0,0,0.7); backdrop-filter: blur(8px);">
        <div class="store-glass-card rounded-2xl max-w-md w-full p-6 sm:p-8 relative animate-scale-in">
            <button type="button" onclick="closeRefundModal()"
                class="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-lg text-white/60 hover:text-white hover:bg-white/10 transition-all">
            ...
            <div class="mb-6">
                ...
                <p class="text-sm text-white/60">Pesanan: <span id="refundOrderNumber"
                        class="text-cyan-300 font-semibold"></span></p>
            </div>
            ...
            <form id="refundForm" method="POST" class="space-y-4">
                ...
                <div class="bg-sky-500/10 border border-sky-500/30 rounded-xl p-3">
                    <div class="flex items-start gap-2">
                        <i class="fas fa-info-circle text-cyan-300 mt-0.5 text-sm"></i>
                        <div>
                            <p class="text-xs font-semibold text-cyan-200 mb-0.5">Informasi:</p>
                            <p class="text-xs text-cyan-300/80 leading-relaxed">Admin akan meninjau permintaan Anda. Jika
                                disetujui, dana akan dikembalikan dan stok produk akan direstore.</p>
                        </div>
                    </div>
                </div>
