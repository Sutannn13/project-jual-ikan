        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <nav class="mb-6 sm:mb-8">
                <ol class="flex items-center gap-2 text-sm">
                    <li><a href="{{ route('home') }}" class="text-white/50 hover:text-cyan-300 transition-colors">Beranda</a>
                    </li>
                    <li class="text-white/30"><i class="fas fa-chevron-right text-[10px]"></i></li>
                    <li><a href="{{ route('catalog') }}"
                            class="text-white/50 hover:text-cyan-300 transition-colors">Katalog</a></li>
                    <li class="text-white/30"><i class="fas fa-chevron-right text-[10px]"></i></li>
            ...
            <div class="grid lg:grid-cols-2 gap-8 lg:gap-12">
                ...
                <div class="relative">
                    ...
                    {{-- Wishlist Button --}}
                    @auth
                        <div class="absolute top-4 left-4" x-data="{
                            ...
                            <button @click="toggle()" :disabled="loading"
                                ...
                                :class="wishlisted ? 'bg-red-500 text-white hover:bg-red-600' :
                                    'bg-white/90 backdrop-blur text-gray-400 hover:text-red-500 hover:bg-white'"
                ...
                <div class="flex flex-col">
                    ...
                    @if ($produk->review_count > 0)
                        <div class="flex items-center gap-2 mb-4">
                            <div class="flex items-center gap-0.5">
                                @for ($i = 1; $i <= 5; $i++)
                                    <i
                                        class="fas fa-star {{ $i <= round($produk->average_rating) ? 'text-amber-400' : 'text-white/20' }}"></i>
                                @endfor
                            </div>
                    ...
                    <div class="flex items-center gap-3 mb-6">
                        <div class="flex items-center gap-2 px-4 py-2 rounded-xl"
                            style="background: {{ $produk->stok > 5 ? 'rgba(16,185,129,0.15)' : ($produk->stok > 0 ? 'rgba(14,165,233,0.15)' : 'rgba(239,68,68,0.15)') }}; border: 1px solid {{ $produk->stok > 5 ? 'rgba(16,185,129,0.25)' : ($produk->stok > 0 ? 'rgba(14,165,233,0.25)' : 'rgba(239,68,68,0.25)') }};">
                            <i
                                class="fas fa-box {{ $produk->stok > 5 ? 'text-mint-400' : ($produk->stok > 0 ? 'text-cyan-300' : 'text-red-400') }}"></i>
                            <span
                                class="font-semibold {{ $produk->stok > 5 ? 'text-mint-300' : ($produk->stok > 0 ? 'text-cyan-200' : 'text-red-300') }}">
                        ...
                        @auth
                            @if ($produk->stok > 0)
                                <div class="store-glass-card rounded-2xl p-6">
                                    ...
                                    <form action="{{ route('cart.add') }}" method="POST" id="orderForm">
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <div class="flex items-center gap-3">
                                                ...
                                                <input type="number" name="qty" id="qty" value="0.5"
                                                    min="0.5" max="{{ $produk->stok }}" step="0.5"
                                                    class="w-24 text-center text-lg font-bold text-white bg-white/10 border border-white/15 rounded-xl py-3 focus:outline-none focus:border-white/30 hide-spinner"
                                        ...
                                        <div
                                            class="flex items-center justify-between mb-5 p-4 rounded-xl bg-white/5 border border-white/10">
                                    ...
                                    <a href="{{ route('cart.index') }}"
                                        class="block text-center mt-4 text-cyan-400 hover:text-cyan-300 font-medium">
                            ...
                            @else
                                <div class="store-glass-card rounded-2xl p-6 text-center">
                                    ...
                                    <p class="text-white/50 text-sm">Produk ini sedang tidak tersedia. Silakan cek kembali
                                        nanti.</p>
                                </div>
                            @endif
                        @else
                        ...
                        @if ($produk->review_count > 0)
                            <div class="flex items-center gap-3 mt-2">
                                <div class="flex items-center gap-1">
                                    @for ($i = 1; $i <= 5; $i++)
                                        <i
                                            class="fas fa-star text-lg {{ $i <= round($produk->average_rating) ? 'text-amber-400' : 'text-white/20' }}"></i>
                                    @endfor
                                </div>
                ...
                @auth
                    @if ($completedOrders->count() > 0)
                        <div class="store-glass-card rounded-2xl p-6 mb-8" x-data="{ rating: 0, hoverRating: 0 }">
                            <h3 class="font-bold text-white mb-4"><i class="fas fa-pen mr-2 text-cyan-400"></i>Tulis Review
                            </h3>
                            <form action="{{ route('review.store', $produk) }}" method="POST">
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="flex items-center gap-1">
                                        ...
                                        @endfor
                                        <span class="ml-2 text-sm text-white/50" x-show="rating > 0"
                                            x-text="['', 'Buruk', 'Kurang', 'Cukup', 'Bagus', 'Sangat Bagus'][rating]"></span>
                                    </div>
                                ...
                                @if ($completedOrders->count() > 1)
                                    <div class="mb-4">
                                        ...
                                        <select name="order_id"
                                            class="w-full px-4 py-3.5 rounded-xl text-sm text-white bg-white/10 border border-white/15 focus:bg-white/15 focus:border-white/30 focus:outline-none">
                                            @foreach ($completedOrders as $co)
                                                <option value="{{ $co->id }}">{{ $co->order_number }}
                                                    ({{ $co->created_at->format('d M Y') }})</option>
                                            @endforeach
                                        </select>
                                    </div>
                                @endif
                                        ...
                                        <div>
                                            ...
                                            <div class="flex items-center gap-2 mt-0.5">
                                                <div class="flex items-center gap-0.5">
                                                    @for ($i = 1; $i <= 5; $i++)
                                                        <i
                                                            class="fas fa-star text-xs {{ $i <= $review->rating ? 'text-amber-400' : 'text-white/20' }}"></i>
                                                    @endfor
                                                </div>
                                                <span
                                                    class="text-xs text-white/40">{{ $review->created_at->diffForHumans() }}</span>
                                            </div>
                                        </div>
                                    </div>
                                    @if (Auth::id() === $review->user_id)
                                        <form action="{{ route('review.destroy', $review) }}" method="POST"
                                            onsubmit="event.preventDefault(); userConfirm(this, 'Hapus Review', 'Yakin ingin menghapus review kamu?', 'danger', 'Ya, Hapus');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                        ...
                        @foreach ($relatedProducts as $related)
                            <div class="product-card flex flex-col">
                                <div class="aspect-[4/3] overflow-hidden bg-white/5">
                                    @if ($related->foto)
                                        <img loading="lazy" src="{{ asset('storage/' . $related->foto) }}"
                                            alt="{{ $related->nama }}"
                                ...
                                <div class="p-4 flex flex-col flex-1">
                                    <span
                                        class="{{ $related->kategori === 'Ikan Nila' ? 'badge-nila' : 'badge-mas' }} w-fit text-xs mb-2">{{ $related->kategori }}</span>
