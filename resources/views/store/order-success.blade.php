...
@section('content')
    <section class="py-12 sm:py-20 relative overflow-hidden">
        ...
        <div class="absolute top-20 left-10 w-64 h-64 rounded-full blur-3xl -z-10 animate-float"
            style="background: rgba(6,182,212,0.08);"></div>
        <div class="absolute bottom-10 right-10 w-80 h-80 rounded-full blur-3xl -z-10 animate-float"
            style="animation-delay: 2s; background: rgba(20,184,166,0.08);"></div>
        ...
        <div class="max-w-2xl mx-auto px-4 text-center">
            ...
            @if ($order->status === 'pending')
                <div class="w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-8 relative group">
                    <div
                        class="absolute inset-0 bg-gradient-to-tr from-sky-400 to-cyan-400 rounded-full opacity-20 group-hover:opacity-30 transition-all duration-500 animate-pulse">
                    </div>
                    <div
                        class="w-20 h-20 bg-gradient-to-tr from-sky-500 to-cyan-500 rounded-full flex items-center justify-center shadow-lg shadow-sky-500/30 transform group-hover:scale-110 transition-all duration-300">
            ...
            @elseif($order->status === 'waiting_payment')
                <div class="w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-8 relative group">
                    <div
                        class="absolute inset-0 bg-gradient-to-tr from-cyan-400 to-teal-400 rounded-full opacity-20 group-hover:opacity-30 transition-all duration-500 animate-pulse">
                    </div>
                    <div
                        class="w-20 h-20 bg-gradient-to-tr from-cyan-500 to-teal-500 rounded-full flex items-center justify-center shadow-lg shadow-cyan-500/30 transform group-hover:scale-110 transition-all duration-300">
            ...
            @else
                <div class="w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-8 relative group">
                    <div
                        class="absolute inset-0 bg-gradient-to-tr from-mint-400 to-teal-400 rounded-full opacity-20 group-hover:opacity-30 transition-all duration-500 animate-pulse">
                    </div>
                    <div
                        class="w-20 h-20 bg-gradient-to-tr from-mint-500 to-teal-500 rounded-full flex items-center justify-center shadow-lg shadow-mint-500/30 transform group-hover:scale-110 transition-all duration-300">
            ...
            <div class="store-glass-card rounded-3xl p-8 mb-10 transform hover:-translate-y-1 transition-all duration-300">
                ...
                <p
                    class="text-4xl sm:text-5xl font-black bg-gradient-to-r from-cyan-300 to-teal-300 bg-clip-text text-transparent mb-6">
                    {{ $order->order_number }}
                </p>
                    ...
                    <div class="mb-6">
                        @if ($order->isPaymentExpired())
                            <div class="rounded-2xl p-4"
                                ...
                                <p class="text-sm text-red-300/70 mt-2">Pesanan akan otomatis dibatalkan. Silakan buat
                                    pesanan baru.</p>
                            </div>
                        @else
                            <div class="rounded-2xl p-4"
                                style="background: rgba(14,165,233,0.08); border: 1px solid rgba(14,165,233,0.2);">
                                <p class="text-xs uppercase tracking-wider text-cyan-300 font-bold mb-2">
                                ...
                                <div id="countdown" class="flex items-center justify-center gap-2">
                                    <div class="rounded-xl px-4 py-2"
                                        ...
                                        <span id="hours" class="text-2xl font-black text-cyan-300">00</span>
                                        <p class="text-[10px] text-white/40 uppercase">Jam</p>
                                    </div>
                                    <span class="text-2xl font-bold text-cyan-500/50">:</span>
                                    <div class="rounded-xl px-4 py-2"
                                        style="background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.1);">
                                        <span id="minutes" class="text-2xl font-black text-cyan-300">00</span>
                                        <p class="text-[10px] text-white/40 uppercase">Menit</p>
                                    </div>
                                    <span class="text-2xl font-bold text-cyan-500/50">:</span>
                                    <div class="rounded-xl px-4 py-2"
                                        style="background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.1);">
                                        <span id="seconds" class="text-2xl font-black text-cyan-300">00</span>
                                        <p class="text-[10px] text-white/40 uppercase">Detik</p>
                                    </div>
                                </div>
                                <p class="text-xs text-cyan-300/70 mt-3">
                ...
                <div class="text-left space-y-4">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="font-semibold text-white text-sm">
                                            {{ $item->nama_produk ?? ($item->produk?->nama ?? 'Produk Dihapus') }}</p>
                                        <p class="text-xs text-white/40">{{ $item->qty }} Kg</p>
                                    </div>
                                </div>
                                <span class="font-bold text-white/70 text-sm">Rp
                    ...
                    <div class="flex justify-between items-center pt-2">
                        ...
                        <span class="text-2xl font-black text-cyan-300">Rp
            ...
            @if ($order->status === 'pending' && $order->rejection_reason)
                <div class="rounded-2xl p-5 mb-8 text-left"
            ...
            @if ($order->status === 'pending' || ($order->status === 'waiting_payment' && $order->wasRejected()))
                ...
                @if ($order->payment_method === 'ewallet_pending')
                    <div class="store-glass-card rounded-3xl p-8 mb-10 text-center">
                        ...
                        <button onclick="payWithMidtrans()" id="pay-button"
                        ...
                        <div class="rounded-xl p-4 mt-6 text-left"
                            style="background: rgba(139,92,246,0.08); border: 1px solid rgba(139,92,246,0.15);">
                            <p class="text-sm text-violet-300">
                                <i class="fas fa-info-circle mr-2"></i>
                                <strong>Pembayaran otomatis & instan!</strong> Setelah Anda membayar, pesanan akan langsung
                                dikonfirmasi tanpa perlu menunggu verifikasi admin.
                            </p>
                        </div>
                ...
                <div class="rounded-2xl p-5 mb-6"
                    ...
                    <div class="flex items-center justify-between rounded-xl p-4"
                        ...
                        <div>
                            ...
                            <p class="text-xl font-black text-cyan-300">Rp
                                {{ number_format($order->total_price, 0, ',', '.') }}</p>
                        </div>
                        <button onclick="copyToClipboard('{{ $order->total_price }}')"
                ...
                <form action="{{ route('order.payment', $order) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div
                        class="border-2 border-dashed border-white/15 rounded-2xl p-6 text-center hover:border-cyan-400/30 transition-colors mb-4">
                        ...
                        <label for="payment_proof" class="cursor-pointer">
                            ...
                            <div id="image-preview" class="hidden">
                                <img loading="lazy" id="preview-img" src="" alt="Preview"
                    ...
                    <div id="file-size-error" class="hidden rounded-xl p-3 text-red-400 text-sm font-medium mb-4"
                        style="background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.2);">
                    </div>

                    @error('payment_proof')
                        <div class="rounded-xl p-3 mb-4"
                            style="background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.2);">
                            <p class="text-red-400 text-sm font-medium"><i
                                    class="fas fa-exclamation-circle mr-1"></i>{{ $message }}</p>
                        </div>
                    @enderror

                    <button type="submit" id="submit-payment-btn" class="btn-primary w-full py-4 text-base btn-shiny"
                        {{ $order->isPaymentExpired() ? 'disabled' : '' }}>
    ...
    @elseif($order->status === 'waiting_payment')
        ...
        <div class="flex items-start gap-3 rounded-2xl p-4 text-left mb-8"
            style="background: rgba(14,165,233,0.1); border: 1px solid rgba(14,165,233,0.2);">
            <div class="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5"
                style="background: rgba(14,165,233,0.15);">
                <i class="fas fa-hourglass-half text-cyan-300"></i>
            </div>
            <div>
            ...
            function previewImage(event) {
                ...
                if (input.files && input.files[0]) {
                    ...
                    if (file.size > MAX_FILE_SIZE) {
                        if (fileSizeError) {
                            fileSizeError.innerHTML = '<i class="fas fa-exclamation-triangle mr-1"></i> Ukuran file ' +
                                fileSizeMB + 'MB terlalu besar! Maksimal 5MB. Coba kompres gambar terlebih dahulu.';
                    ...
                    if (!allowedTypes.includes(file.type)) {
                        if (fileSizeError) {
                            fileSizeError.innerHTML =
                                '<i class="fas fa-exclamation-triangle mr-1"></i> Format file tidak didukung! Gunakan format JPG atau PNG.';
                            fileSizeError.classList.remove('hidden');
                        }
                        if (submitBtn) submitBtn.disabled = true;
                        input.value = '';
                    ...
                    function updateCountdown() {
                        ...
                        if (distance < 0) {
                            document.getElementById('countdown').innerHTML =
                                '<span class="text-red-600 font-bold">Waktu Habis!</span>';
                            clearInterval(timer);
