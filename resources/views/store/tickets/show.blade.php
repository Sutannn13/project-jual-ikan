        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6">
                <a href="{{ route('tickets.index') }}"
                    class="inline-flex items-center gap-2 text-white/60 hover:text-white text-sm mb-4 transition-colors">
            ...
            <div class="space-y-4 mb-6">
                @foreach ($ticket->messages as $msg)
                    <div class="flex {{ $msg->is_admin ? 'justify-start' : 'justify-end' }}">
                        <div
                            class="max-w-[85%] {{ $msg->is_admin ? 'store-glass-card' : 'bg-cyan-600/20 border border-cyan-500/30' }} rounded-2xl p-4 sm:p-5">
                            <div class="flex items-center gap-2 mb-2">
                                @if ($msg->is_admin)
                                    <div
                                        class="w-7 h-7 rounded-full bg-gradient-to-br from-cyan-500 to-teal-500 flex items-center justify-center text-white text-xs font-bold">
                                        <i class="fas fa-shield-alt text-[10px]"></i>
                                    </div>
                                    <span class="text-sm font-semibold text-cyan-300">Admin</span>
                                @else
                                    <div
                                        class="w-7 h-7 rounded-full bg-gradient-to-br from-cyan-500 to-blue-500 flex items-center justify-center text-white text-xs font-bold">
                                        {{ substr($msg->user->name ?? 'U', 0, 1) }}
                                    </div>
                                    <span
                                        class="text-sm font-semibold text-cyan-300">{{ $msg->user->name ?? 'Anda' }}</span>
                                @endif
                                <span
                                    class="text-[10px] text-white/40 ml-auto">{{ $msg->created_at->format('d/m H:i') }}</span>
                            </div>
            ...
            @else
                <div class="store-glass-card rounded-2xl p-5 text-center">
                    <p class="text-white/50 text-sm">
                        <i class="fas fa-lock mr-1"></i> Tiket ini sudah
                        {{ $ticket->status === 'closed' ? 'ditutup' : 'diselesaikan' }}.
                    </p>
                </div>
            @endif
        </div>
