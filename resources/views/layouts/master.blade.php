...
<body class="min-h-screen flex flex-col text-white overflow-x-hidden" x-data="{
    mobileOpen: false,
    bubbles: [],
    init() {
    ...
    {{-- ========================================
         ...
         ======================================== --}}
    <nav class="nav-blur sticky top-0 z-50 border-b border-white/20" x-data="{ scrolled: false }"
        @scroll.window="scrolled = (window.pageYOffset > 20)" :class="{ 'scrolled': scrolled }">
        <div
            class="absolute inset-0 bg-gradient-to-r from-white/10 via-white/5 to-white/10 backdrop-blur-2xl transition-all duration-300">
        </div>
        ...
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16 sm:h-18">
                ...
                <a href="{{ route('home') }}" class="flex items-center gap-2.5 group">
                    ...
                    <span
                        class="text-xl font-bold bg-gradient-to-r from-white via-cyan-100 to-white bg-clip-text text-transparent drop-shadow-2xl">
                        FishMarket
                    </span>
                </a>
                ...
                <div class="hidden md:flex items-center gap-2">
                    <a href="{{ route('home') }}"
                        ...
                        @if (request()->routeIs('home'))
                            <div
                                class="absolute inset-0 rounded-xl bg-gradient-to-r from-cyan-400/20 to-teal-400/20 blur-sm">
                            </div>
                        @endif
                    </a>
                ...
                <div class="hidden md:flex items-center gap-3">
                    @guest
                        <a href="{{ route('login') }}"
                            class="px-5 py-2.5 text-sm font-semibold text-white/90 hover:text-white hover:bg-white/10 rounded-xl transition-all duration-300">
                            Masuk
                        </a>
                        <a href="{{ route('register') }}"
                            class="relative group px-6 py-2.5 text-sm font-bold text-white rounded-xl overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-teal-500/40">
                            <div class="absolute inset-0 bg-gradient-to-r from-cyan-500 to-teal-500"></div>
                            <div
                                class="absolute inset-0 bg-gradient-to-r from-teal-500 to-cyan-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            </div>
                    ...
                    @else
                        <div class="relative" x-data="{ open: false }" @keydown.escape.window="open = false">
                            <button @click="open = !open"
                                ...
                                <div class="relative w-10 h-10 rounded-full flex items-center justify-center overflow-hidden ring-2 ring-white/30 group-hover:ring-cyan-400/60 transition-all"
                                    ...
                                    @if (Auth::user()->foto_profil)
                                        <img loading="lazy" src="{{ asset('storage/' . Auth::user()->foto_profil) }}"
                                ...
                                <span
                                    class="hidden sm:block max-w-[120px] truncate text-white font-semibold">{{ Auth::user()->name }}</span>
                                <i class="fas fa-chevron-down text-xs text-white/70 transition-transform"
                            ...
                            <div x-show="open" @click.outside="open = false"
                                ...
                                <div
                                    class="absolute inset-0 bg-gradient-to-br from-cyan-500/10 via-transparent to-teal-500/10">
                                </div>
                                @if (Auth::user()->isAdmin())
                                    <a href="{{ route('admin.dashboard') }}"
                                        class="relative flex items-center gap-3 px-4 py-3 text-sm text-white/90 hover:bg-white/15 hover:text-white transition-all">
                                        <i class="fas fa-tachometer-alt w-5 text-center text-cyan-400"></i>
                                        <span class="font-medium">Dashboard Admin</span>
                                    </a>
                                    <div class="h-px bg-gradient-to-r from-transparent via-white/20 to-transparent my-2">
                                    </div>
                                @endif
                                ...
                                <form action="{{ route('logout') }}" method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="relative flex items-center gap-3 px-4 py-3 text-sm text-red-400 hover:bg-red-500/20 w-full text-left transition-all font-medium">
                ...
                <div class="md:hidden flex items-center gap-2">
                    ...
                    <button @click="mobileOpen = !mobileOpen"
                        ...
                        <i class="fas text-lg transition-transform"
                            :class="mobileOpen ? 'fa-times rotate-90' : 'fa-bars'"></i>
                    </button>
                </div>
            </div>
            ...
            <div x-show="mobileOpen" x-transition:enter="transition ease-out duration-300"
                ...
                <div class="space-y-2 p-3 rounded-2xl"
                    style="background: linear-gradient(135deg, rgba(6, 182, 212, 0.15) 0%, rgba(20, 184, 166, 0.15) 100%); backdrop-filter: blur(20px); border: 1px solid rgba(255,255,255,0.15);">
                    <a href="{{ route('home') }}" @click="mobileOpen = false"
                        class="flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-semibold transition-all {{ request()->routeIs('home') ? 'bg-gradient-to-r from-cyan-500/40 to-teal-500/40 text-white border border-white/20 shadow-lg' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
                        <i class="fas fa-home w-5 text-center"></i> Beranda
                    </a>
                    <a href="{{ route('catalog') }}" @click="mobileOpen = false"
                        class="flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-semibold transition-all {{ request()->routeIs('catalog') ? 'bg-gradient-to-r from-cyan-500/40 to-teal-500/40 text-white border border-white/20 shadow-lg' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
                        <i class="fas fa-fish w-5 text-center"></i> Katalog
                    </a>
                    @auth
                        <a href="{{ route('cart.index') }}" @click="mobileOpen = false"
                            class="flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-semibold transition-all {{ request()->routeIs('cart.*') ? 'bg-gradient-to-r from-cyan-500/40 to-teal-500/40 text-white border border-white/20 shadow-lg' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
                        ...
                        <a href="{{ route('my.orders') }}" @click="mobileOpen = false"
                            class="flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-semibold transition-all {{ request()->routeIs('my.orders') ? 'bg-gradient-to-r from-cyan-500/40 to-teal-500/40 text-white border border-white/20 shadow-lg' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
                            <i class="fas fa-box w-5 text-center"></i> Pesanan Saya
                        </a>
                        <a href="{{ route('wishlist.index') }}" @click="mobileOpen = false"
                            class="flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-semibold transition-all {{ request()->routeIs('wishlist.*') ? 'bg-gradient-to-r from-cyan-500/40 to-teal-500/40 text-white border border-white/20 shadow-lg' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
                            <i class="fas fa-heart w-5 text-center"></i> Wishlist
                        </a>
                        <a href="{{ route('profile.show') }}" @click="mobileOpen = false"
                            class="flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-semibold transition-all {{ request()->routeIs('profile.*') ? 'bg-gradient-to-r from-cyan-500/40 to-teal-500/40 text-white border border-white/20 shadow-lg' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
                            <i class="fas fa-user-edit w-5 text-center"></i> Profil
                        </a>
                        <a href="{{ route('chat.index') }}" @click="mobileOpen = false"
                            class="flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-semibold transition-all {{ request()->routeIs('chat.*') ? 'bg-gradient-to-r from-cyan-500/40 to-teal-500/40 text-white border border-white/20 shadow-lg' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
                            <i class="fas fa-comments w-5 text-center"></i> Chat Admin
                        </a>
                        <a href="{{ route('tickets.index') }}" @click="mobileOpen = false"
                            class="flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-semibold transition-all {{ request()->routeIs('tickets.*') ? 'bg-gradient-to-r from-cyan-500/40 to-teal-500/40 text-white border border-white/20 shadow-lg' : 'text-white/80 hover:bg-white/10 hover:text-white' }}">
                            <i class="fas fa-headset w-5 text-center"></i> Support Ticket
                        </a>
                        @if (Auth::user()->isAdmin())
                            <div class="h-px bg-gradient-to-r from-transparent via-white/20 to-transparent my-2"></div>
                            <a href="{{ route('admin.dashboard') }}" @click="mobileOpen = false"
                                class="flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-semibold text-white/80 hover:bg-white/10 hover:text-white transition-all">
                        ...
                        <div class="pt-3 mt-3" style="border-top: 1px solid rgba(255,255,255,0.15);">
                            <form action="{{ route('logout') }}" method="POST">
                                @csrf
                                <button type="submit"
                                    class="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-semibold bg-gradient-to-r from-red-500/20 to-rose-500/20 text-red-300 hover:from-red-500/30 hover:to-rose-500/30 hover:text-white transition-all border border-red-400/20">
                                    <i class="fas fa-sign-out-alt w-5 text-center"></i> Logout
                                </button>
                            </form>
                        </div>
                    @else
                        <div class="flex gap-3 pt-4 mt-3" style="border-top: 1px solid rgba(255,255,255,0.15);">
                            <a href="{{ route('login') }}"
                                class="flex-1 text-sm text-center py-3.5 rounded-xl font-bold text-white/90 border border-white/30 hover:bg-white/15 transition-all">
                                Masuk
                            </a>
                            <a href="{{ route('register') }}"
                                class="flex-1 text-sm text-center py-3.5 rounded-xl font-bold text-white shadow-lg transition-all hover:scale-105"
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-5">
            @if (session('success'))
                <div class="relative flex items-center gap-4 px-5 py-4 rounded-2xl text-sm font-semibold overflow-hidden shadow-xl"
                    ...
                    <button @click="show = false"
                        class="relative ml-auto text-white/70 hover:text-white transition-colors">
            ...
            @if (session('error'))
                <div class="relative flex items-center gap-4 px-5 py-4 rounded-2xl text-sm font-semibold overflow-hidden shadow-xl"
                    ...
                    <div class="relative w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                        style="background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%); box-shadow: 0 4px 15px rgba(239,68,68,0.4);">
            ...
            @if (session('warning'))
                <div class="relative flex items-center gap-4 px-5 py-4 rounded-2xl text-sm font-semibold overflow-hidden shadow-xl"
                    ...
                    <div class="relative w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                        style="background: linear-gradient(135deg, #fb923c 0%, #f97316 100%); box-shadow: 0 4px 15px rgba(251,146,60,0.4);">
    ...
    <footer class="mt-auto relative overflow-hidden">
        ...
        <div class="absolute inset-0 opacity-40">
            <div class="absolute top-0 left-1/4 w-96 h-96 bg-cyan-500/30 rounded-full blur-[100px] animate-pulse">
            </div>
            <div class="absolute bottom-0 right-1/4 w-80 h-80 bg-teal-500/30 rounded-full blur-[100px] animate-pulse"
                style="animation-delay: 1s;"></div>
            <div
                class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-sky-500/20 rounded-full blur-[120px]">
            </div>
        </div>
        ...
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 sm:gap-12">
                ...
                <div>
                    <div class="flex items-center gap-3 mb-5">
                        <div class="relative w-12 h-12 rounded-xl flex items-center justify-center"
                            ...
                            <div class="absolute inset-0 rounded-xl bg-gradient-to-br from-white/20 to-transparent">
                            </div>
                            <i class="fas fa-fish text-white text-xl relative z-10"></i>
                        </div>
                        <span
                            class="text-2xl font-bold bg-gradient-to-r from-white via-cyan-100 to-white bg-clip-text text-transparent">FishMarket</span>
                    </div>
                    <p class="text-cyan-100/80 text-sm leading-relaxed">
                        Marketplace ikan air tawar terpercaya. Ikan Nila & Ikan Mas berkualitas langsung dari kolam
                        petani.
                    </p>
                    <div class="mt-5 flex gap-3">
                        <a href="#"
                            class="w-10 h-10 rounded-lg flex items-center justify-center text-white/70 hover:text-white hover:bg-white/15 transition-all duration-300 hover:scale-110"
                ...
                <div>
                    ...
                    <ul class="space-y-3">
                        <li>
                            <a href="{{ route('home') }}"
                                class="flex items-center gap-2 text-cyan-100/80 text-sm hover:text-white hover:translate-x-1 transition-all duration-300">
                ...
                <div>
                    ...
                    <ul class="space-y-4 text-sm text-cyan-100/80">
                        <li class="flex items-start gap-3 group hover:translate-x-1 transition-transform duration-300">
                            <div class="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 group-hover:bg-cyan-500/20 transition-colors"
                                style="background: rgba(6, 182, 212, 0.15);">
            ...
            <div class="relative mt-12 pt-8">
                <div
                    class="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-cyan-400/30 to-transparent">
                </div>
    ...
    @auth
        @if (!Auth::user()->isAdmin())
            <div class="fixed bottom-6 right-6 z-50" x-data="{ unread: 0 }" x-init="setInterval(() => {
            ...
            <div class="fixed inset-0 z-[9999] flex items-center justify-center px-4"
                ...
                <div class="relative w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl transform"
                    ...
                    <div class="absolute top-0 left-1/2 -translate-x-1/2 w-40 h-1 rounded-b-full"
                        :class="{
                            'bg-gradient-to-r from-transparent via-red-400 to-transparent': confirm
                                .type === 'danger',
                            'bg-gradient-to-r from-transparent via-amber-400 to-transparent': confirm
                                .type === 'warning',
                            'bg-gradient-to-r from-transparent via-cyan-400 to-transparent': confirm
                                .type === 'info',
                            'bg-gradient-to-r from-transparent via-teal-400 to-transparent': confirm
                                .type === 'success'
                        }">
                    </div>
                    <div class="p-7">
                        ...
                        {{-- Message --}}
                        <p class="text-cyan-200/50 text-center text-sm leading-relaxed mb-7" x-text="confirm.message">
                        </p>
                        {{-- Actions --}}
                        <div class="flex items-center gap-3">
                            ...
                            <button @click="proceedConfirm()"
                                ...
                                :class="{
                                    'bg-gradient-to-r from-red-500 to-rose-500 shadow-red-500/30': confirm
                                        .type === 'danger',
                                    'bg-gradient-to-r from-amber-500 to-orange-500 shadow-amber-500/30': confirm
                                        .type === 'warning',
                                    'bg-gradient-to-r from-cyan-500 to-teal-500 shadow-cyan-500/30': confirm
                                        .type === 'info',
                                    'bg-gradient-to-r from-teal-500 to-emerald-500 shadow-teal-500/30': confirm
                                        .type === 'success'
                                }"
